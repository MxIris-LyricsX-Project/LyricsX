...
//        image.draw(in: dirtyRect)
        ...
        if drawRomajin && !romajinAnnotations.isEmpty {
            let lines = ctFrame().lines
            let origins = ctFrame().lineOrigins(range: CFRangeMake(0, lines.count))
                ...
                for run in runs {
                    ...
                    while annotationIndex + subIndex < romajinAnnotations.count {
                        ...
                        if NSRange(location: range.location, length: range.length).contains(annotationRange.location) {
                            ...
                            cgContext.textPosition = rubyPoint
                            CTLineDraw(rubyLine, cgContext)
